package edu.hendrix.csci235.ideas.vision;

import java.io.File;
import java.io.FileNotFoundException;

import edu.hendrix.modeselection.util.Util;
import edu.hendrix.modeselection.vision.AdaptedYUYVImage;
import edu.hendrix.modeselection.vision.BitImage;
import edu.hendrix.modeselection.vision.config.VisionBot;
import lejos.hardware.lcd.LCD;

public class SceneViewer extends VisionBot {
	public static final String FILENAME = "img1.txt";

	private BitImage ref;
	private int total, min, max;
	
	public static void main(String[] args) throws FileNotFoundException {
		SceneViewer checker = new SceneViewer(FILENAME);
		checker.run();
	}
	
	public SceneViewer(String filename) throws FileNotFoundException {
		ref = Util.fileToObject(new File(filename), s -> BitImage.from(s));
		min = Integer.MAX_VALUE;
		max = Integer.MIN_VALUE;
		total = 0;
	}
	
	@Override
	public BitImage processImage(AdaptedYUYVImage img) {
		BitImage proc = BitImage.intensityView(img);
		BitImage xored = proc.xored(ref);
		int xor = xored.size();
		if (xor < min) {min = xor;}
		if (xor > max) {max = xor;}
		total += xor;
		return xored;
	}
	
	@Override
	public void displayFinalInfo() {
		displayFrameRate(2);
		LCD.drawString(String.format("min: %d", min), 0, 3);
		LCD.drawString(String.format("max: %d", max), 0, 4);
		LCD.drawString(String.format("mean:%d", total/getCycles()), 0, 5);		
	}
}
